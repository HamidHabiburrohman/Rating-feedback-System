...
@section('admin-content')
    ...
    <div class="ud-toast-wrap" id="toastContainer">
        @if(session('success'))
            <div class="ud-toast ud-toast--success" role="alert">
                <svg class="ud-toast__icon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
            ...
            <div class="col-lg-7 col-xl-8">
                ...
                <div class="ud-card ud-card--hero mb-4">
                    ...
                    @if($unit->primaryPhoto)
                        <div class="ud-hero">
                            <img src="{{ asset('storage/' . $unit->primaryPhoto->original_path) }}"
                                 alt="{{ $unit->name }}" class="ud-hero__img">
                    ...
                    @else
                        <div class="ud-hero ud-hero--empty">
                            <div class="ud-hero__placeholder">
                                ...
                                @else
                                    <svg width="52" height="52" viewBox="0 0 24 24" fill="none" stroke="var(--ud-primary)" stroke-width="1.25">
                                ...
                                @foreach($unit->facilities as $facility)
                                    <span class="ud-facility-chip">
                                        @if($facility->icon_key)
                                            <svg class="ud-facility-chip__icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                {!! $facility->icon_key !!}
                                            </svg>
                                        @endif
                                        {{ $facility->name }}
                                ...
                                @if($unit->phone || $unit->email)
                                    <div class="col-md-6">
                                        <div class="ud-infoblock">
                                            ...
                                            <div class="ud-infoblock__rows">
                                                @if($unit->phone)
                                                    <div class="ud-row"><span>Telepon</span><a href="tel:{{ $unit->phone }}">{{ $unit->phone }}</a></div>
                                                @endif
                                                @if($unit->email)
                                                    <div class="ud-row"><span>Email</span><a href="mailto:{{ $unit->email }}">{{ $unit->email }}</a></div>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                ...
                                @if($unit->open_time || $unit->close_time)
                                    <div class="col-md-6">
                                        <div class="ud-infoblock">
                                            ...
                                            <div class="ud-infoblock__rows">
                                                @if($unit->open_time)
                                                    <div class="ud-row"><span>Buka</span><span>{{ \Carbon\Carbon::parse($unit->open_time)->format('H:i') }}</span></div>
                                                @endif
                                                @if($unit->close_time)
                                                    <div class="ud-row"><span>Tutup</span><span>{{ \Carbon\Carbon::parse($unit->close_time)->format('H:i') }}</span></div>
                                                @endif
                                                @if($isOpen !== null)
                                                    <div class="ud-row">
                                                        <span>Status</span>
                                                        <span class="ud-openpill ud-openpill--{{ $isOpen ? 'open' : 'closed' }}" style="font-size:0.72rem;padding:2px 10px;">
                                                            <span class="ud-openpill__dot"></span>
                                                            {{ $isOpen ? 'Buka' : 'Tutup' }}
                                                        </span>
                                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="ud-infoblock">
                                            ...
                                            <div class="ud-infoblock__rows">
                                                ...
                                                @endif
                                                @if($unit->department)
                                                    <div class="ud-row"><span>Departemen</span><span>{{ $unit->department->name }}</span></div>
                                                @endif
                                                @if($unit->capacity)
                                                    <div class="ud-row"><span>Kapasitas</span><span>{{ number_format($unit->capacity) }} orang</span></div>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="ud-card__body">
                        ...
                        <div class="ud-techlist">
                            ...
                            <div class="ud-techrow">
                                ...
                                <span class="ud-status-chip ud-status-chip--{{ $unit->is_active ? 'active' : 'inactive' }}" style="font-size:0.7rem;padding:2px 10px;">
                                    <span class="ud-status-chip__dot"></span>
                                    {{ $unit->is_active ? 'Aktif' : 'Nonaktif' }}
                                </span>
                            </div>
                            ...
                            <div class="ud-techlist ud-techlist--dark">
                                @if($unit->open_time)
                                    <div class="ud-techrow">
                                        ...
                                        <span class="ud-techrow__val">{{ \Carbon\Carbon::parse($unit->open_time)->format('H:i') }}</span>
                                    </div>
                                @endif
                                @if($unit->close_time)
                                    <div class="ud-techrow">
                                        <span class="ud-techrow__key">Tutup</span>
                                        <span class="ud-techrow__val">{{ \Carbon\Carbon::parse($unit->close_time)->format('H:i') }}</span>
                                    </div>
                                @endif
                                @if($isOpen !== null)
                                    <div class="ud-techrow ud-techrow--last">
                    ...
                    <div class="ud-card__body">
                        ...
                        <div class="ud-navlist">
                            <a href="{{ route('admin.units.index') }}" class="ud-navlink">
                                ...
                                Semua Unit
                                <svg class="ud-navlink__arrow" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <style>
        ...
        @media (min-width: 1200px) {
            .ud-card__body { padding: 1.75rem 2rem; }
        }
        .ud-card__header {
            display: flex;
            align-items: center;
        ...
        @media (min-width: 1200px) {
            .ud-identity { padding: 1.5rem 2rem 2rem; }
        }
        .ud-identity__top {
            display: flex;
        ...
        @media (min-width: 1200px) {
            .ud-identity__name { font-size: 2.25rem; }
        }
        ...
        .ud-attr--star { color: #b45309; }
        .ud-attr__sub { color: var(--ud-text-4); font-weight: 400; }
        ...
        .ud-row > span:first-child, .ud-row > a:first-child {
            font-size: 0.78rem;
            color: var(--ud-text-4);
            flex-shrink: 0;
        }
        ...
        .ud-row > a:last-child:hover { text-decoration: underline; }
        ...
        .ud-stat__num--star { color: #b45309; }
        .ud-stat__label {
            font-size: 0.65rem;
            font-weight: 700;
            text-transform: uppercase;
        ...
        .ud-techrow--last { border-bottom: none; }
        .ud-techlist--dark .ud-techrow { border-bottom-color: var(--ud-dark-border); }
        .ud-techrow__key {
            font-size: 0.78rem;
            color: var(--ud-text-4);
        }
        .ud-techlist--dark .ud-techrow__key { color: var(--ud-dark-muted); }
        .ud-techrow__val {
            font-size: 0.82rem;
            font-weight: 600;
            color: var(--ud-text-1);
        ...
        .ud-techlist--dark .ud-techrow__val { color: #f1f5f9; }
        ...
        .ud-toast--success { border-left-color: #10b981; }
        .ud-toast--success .ud-toast__icon { color: #10b981; }
        .ud-toast--error   { border-left-color: #ef4444; }
        .ud-toast--error .ud-toast__icon   { color: #ef4444; }
        .ud-toast__msg {
            flex: 1;
            font-size: 0.82rem;
            font-weight: 500;
        ...
        @media (max-width: 768px) {
            .ud-page { padding: 1rem 1rem 2.5rem; }
            .ud-identity__name { font-size: 1.625rem; }
            .ud-hero { height: 260px; }
            .ud-stat__num { font-size: 2rem; }
        }
    </style>
