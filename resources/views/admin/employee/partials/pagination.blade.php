...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-center py-3" style="border-color: #e5e7eb;">
    <div class="mb-3 mb-md-0">
        <p class="mb-0 text-muted small">
            Showing {{ ($employees->currentPage() - 1) * $employees->perPage() + 1 }} to 
            {{ min($employees->currentPage() * $employees->perPage(), $employees->total()) }} of 
    ...
    <nav aria-label="Page navigation">
        <ul class="pagination mb-0" style="gap: 2px;">
            ...
            <li class="page-item {{ $employees->onFirstPage() ? 'disabled' : '' }}">
                <a class="page-link border-0" href="{{ $employees->previousPageUrl() }}" style="border-radius: 6px;">
            ...
            @for($i = $start; $i <= $end; $i++)
                <li class="page-item {{ $i == $current ? 'active' : '' }}">
                    ...
                    @else
                        <a class="page-link border-0" href="{{ $employees->url($i) }}" style="border-radius: 6px;">{{ $i }}</a>
                    @endif
                </li>
            @endfor

            <li class="page-item {{ !$employees->hasMorePages() ? 'disabled' : '' }}">
                <a class="page-link border-0" href="{{ $employees->nextPageUrl() }}" style="border-radius: 6px;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
@elseif($employees instanceof \Illuminate\Pagination\LengthAwarePaginator)
<div class="d-flex justify-content-center py-4" style="border-color: #e5e7eb;">
    <p class="text-muted small mb-0">Showing all {{ $employees->total() }} results</p>
</div>
@endif