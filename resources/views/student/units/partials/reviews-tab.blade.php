...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-20">
    <div class="lg:col-span-5 bg-surface-container-lowest p-12 rounded-2xl shadow-lg text-center border border-outline-variant/10">
        ...
        <p class="text-on-surface-variant font-medium">Based on {{ $totalRatings }} Student {{ Str::plural('Review', $totalRatings) }}</p>
    </div>
    ...
    <div class="lg:col-span-7 space-y-6">
        @forelse($categories as $category)
            ...
            <div class="flex items-center gap-6">
                <span class="w-32 text-sm font-bold text-on-surface-variant uppercase tracking-tighter">
                    {{ $category['name'] }}
                </span>
                ...
                <span class="w-10 text-sm font-bold text-right">{{ number_format($categoryScore, 1) }}</span>
            </div>
        @empty
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    @forelse($ratings as $review)
        <div class="bg-surface-container-lowest p-8 rounded-xl border transition-all duration-300 hover:shadow-lg group">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        @if($review->status === 'edited')
                            <span class="text-xs text-primary/70 mt-1 inline-block">(Edited)</span>
                        @endif
                    </div>
                </div>
            ...
            @if($review->scores->count())
                <div class="flex flex-wrap gap-3 pt-4">
                    @foreach($review->scores as $score)
                        <div class="flex items-center gap-1.5 text-on-surface-variant bg-surface-container-low px-3 py-1.5 rounded-full">
            ...
            @if($review->adminReply)
                <div class="mt-6 pt-4 border-t bg-surface-container-low rounded-lg p-4">
...
@auth('student')
    @php
        ...
        $hasReviewed = $unit->ratings()
            ->where('student_identifier', $studentIdentifier)
            ->where('status', '!=', 'archived')
            ->exists();
        $existingReview = $unit->ratings()
            ->where('student_identifier', $studentIdentifier)
            ->first();
    @endphp

    @if(!$hasReviewed)
        <div class="mt-12 bg-primary/5 p-10 rounded-2xl border-2 border-dashed border-primary/30 flex flex-col justify-center items-center text-center">
            ...
            <p class="text-on-surface-variant mb-8 max-w-md">Share your experience to help other researchers and improve our facilities.</p>
    ...
    @else
        <div class="mt-12 bg-surface-container-low p-10 rounded-2xl border border-outline-variant/10 flex flex-col justify-center items-center text-center">
