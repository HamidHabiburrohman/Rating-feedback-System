...
#replyModal{{ $rating->id }} .modal-dialog { max-width: 560px; }
...
#replyModal{{ $rating->id }} .rrm-card-info { flex: 1; min-width: 0; }
...
#replyModal{{ $rating->id }} .rrm-response-label .req { color: #f8773c; }
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rrm-wrap">
            ...
            <div class="rrm-body">
                        ...
                        <div class="rrm-card-info">
                            ...
                            <div class="rrm-meta-row">
                                <span class="rrm-meta-item">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.75">
                                    ...
                                    @for($i = 1; $i <= 5; $i++)
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style="margin-right: 2px;">
                                            <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"
                                                     fill="{{ $i <= round($rating->overall_score ?? 0) ? '#f8773c' : '#e5e7eb' }}" stroke="none"/>
                                        </svg>
                                    @endfor
                                </div>
                ...
                <form action="{{ route('admin.admin-replies.store', $rating->id) }}" method="POST" id="replyForm{{ $rating->id }}">
                    @csrf
                    ...
                    <div class="quick-responses">
                        <button type="button" class="quick-response" data-target="reply_message_{{ $rating->id }}"
                            data-response="Terima kasih atas rating positifnya. Kami senang Anda puas dengan pelayanan kami.">Terima kasih</button>
                        <button type="button" class="quick-response" data-target="reply_message_{{ $rating->id }}"
                            data-response="Terima kasih atas masukannya. Kami akan segera menindaklanjuti untuk perbaikan.">Akan ditindaklanjuti</button>
                        <button type="button" class="quick-response" data-target="reply_message_{{ $rating->id }}"
                            data-response="Mohon maaf atas ketidaknyamanannya. Kami akan berusaha meningkatkan kualitas pelayanan.">Permintaan maaf</button>
                    </div>
                </form>
            </div>
