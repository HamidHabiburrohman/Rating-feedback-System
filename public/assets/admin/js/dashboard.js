...
$(document).ready(function () {
    ...
    const routes = {
        ...
        topEmployees: container.dataset.topEmployees
    };
...
function initEventListeners(routes) {
    $('.unit-filter-tab').on('click', function () {
        $('.unit-filter-tab').removeClass('active');
        $(this).addClass('active');
        currentUnitFilter = $(this).data('filter');
        currentPage = 1;
        loadTopUnits(currentUnitFilter, routes);
    });

    $('.employee-filter-tab').on('click', function () {
        $('.employee-filter-tab').removeClass('active');
        $(this).addClass('active');
        currentEmployeeFilter = $(this).data('filter');
        loadTopEmployees(currentEmployeeFilter, routes);
    });
...
function initCharts() {
    ...
    if (chartStudentsEl) {
        charts.students = new ApexCharts(chartStudentsEl, studentOptions);
        charts.students.render();
    }

    const unitsMonthlyOptions = {
        series: [
            { name: 'New Units', type: 'bar', data: [] },
            { name: 'Cumulative', type: 'line', data: [] }
        ],
        chart: {
            type: 'line',
            height: 350,
...
function loadAllData(routes) {
    fetch(routes.stats)
        .then(res => res.json())
        .then(data => { if (data.success) updateStats(data.data); });
...
function updateStats(stats) {
    ...
    animateValue('#avg-rating', stats.avg_rating || 0, true);
    animateValue('#active-units', stats.active_units || 0);
    animateValue('#total-employees', stats.total_employees || 0);
    animateValue('#active-employees', stats.active_employees || 0);
}

function updateCharts(data) {
    if (data.student_login_trend && charts.students) {
        ...
        charts.students.updateOptions({ xaxis: { categories: categories } });
        charts.students.updateSeries([{ data: values }]);
    }

    if (data.unit_growth && charts.unitsMonthly) {
        const months = data.unit_growth.months || [];
        const newUnits = data.unit_growth.new_units || [];
        const cumulative = data.unit_growth.cumulative || [];

        if (months.length > 0) {
            charts.unitsMonthly.updateOptions({ xaxis: { categories: months } });
            charts.unitsMonthly.updateSeries([
                { name: 'New Units', data: newUnits },
                { name: 'Cumulative', data: cumulative }
            ]);
            $('#total-units-display').text(cumulative[cumulative.length - 1] || 0);
        }
    }
}
...
function loadTopUnits(filter, routes) {
    ...
    let url;
    switch (filter) {
        case 'popularity': url = '/admin/dashboard/top-units/popularity?limit=50'; break;
        case 'quality': url = '/admin/dashboard/top-units/quality?limit=50'; break;
        case 'attention': url = '/admin/dashboard/attention-units?limit=50'; break;
        case 'all':
        default: url = '/admin/dashboard/top-units/all?limit=50'; break;
    }
    ...
    tbody.innerHTML = units.map((unit) => {
        ...
        return `
            <tr>
                ...
                <td>
                    <span class="badge-category">${unit.type?.name || 'General'}</span>
                </td>
                <td>
...
function loadRecentRated(routes) {
    ...
    fetch(routes.recentRated)
        ...
        .catch(error => {
            ...
            container.innerHTML = `<div class="rated-empty">Failed to load ratings.</div>`;
        });
}
...
function loadTopEmployees(filter, routes) {
    ...
    tbody.innerHTML = employees.map((emp) => {
        ...
        return `
            <tr>
                <td>
                    <div class="unit-cell">
                        ...
                        <div class="unit-info">
                            ...
                            <div class="unit-type">${emp.position || 'Staff'}</div>
                        </div>
                    </div>
                </td>
                <td>${unitsHtml}</td>
                ...
                <td class="text-center">
                    <button class="btn-action" onclick="viewEmployee(${emp.id})" title="View">
                        <i class="ti ti-eye"></i>
                    </button>
                    <button class="btn-action" onclick="editEmployee(${emp.id})" title="Edit">
                        <i class="ti ti-edit"></i>
                    </button>
                </td>
            </tr>
...
function animateValue(selector, end, isDecimal = false) {
...
function viewEmployee(id) { window.location.href = `/admin/employees/${id}`; }
function editEmployee(id) { window.location.href = `/admin/employees/${id}/edit`; }
